<template>
  <!-- Section 1 -->
  <section class="w-full bg-cgray-100 bg-opacity-50 xl:px-0">
    <div class="py-16 max-w-6xl mx-auto px-4 xl:px-0">
      <div class="flex flex-col md:flex-row">
        <div class="w-full pr-5 md:w-10/12 xl:pr-12">
          <h3 class="text-2xl sm:text-3xl font-extrabold sm:leading-[2.4rem]">
            Fondée sur <mark class="mark1"> l’innovation</mark> dans la production et
            l’offre de ses services
          </h3>
        </div>

        <div class="w-full mt-5 md:mt-0 md:w-4/5 md:pl-6">
          <p class="text-md text-cgray-700 font-normal text-gray-700 md:text-lg">
            Amani Innovation se veut d’accroitre la compétitivité au sein de la province
            et de la ville de Bukavu en s’éloignant de l’amateurisme prédominant sur le
            marché bukavien obstruant le succès et le développement de la ville toute
            entière.
          </p>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: "AboutS1",
};
</script>
